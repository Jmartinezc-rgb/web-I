...
// Para que el usuario pueda actualizar su puntuación
app.post('/update-score', (req, res) => {
  ...
  if (!score || isNaN(score)) {
      return res.status(400).send({ error: 'Puntuación inválida' });
  }

  try {
      users.updateScore(req.session.user, parseInt(score));
      res.redirect('/restricted'); // Redirige a la página de puntuaciones
  } catch (error) {
